...
from .base import InstitutionProfile, Capability
...
PROFILE = InstitutionProfile(
    key="ufcg", label="UFCG (provisional)", host=HOST,
    ...
    ava_url=BASE + "/ava/index.jsf", curriculum_entry_url="", curriculum_data_url="",
    matricula_instrucoes_url="", matricula_turmas_curriculo_url="",
    auth_marker="dispatch=logOff", login_redirect_marker="verTelaLogin.do",
...
class UfcgNavigator:
    ...
    def looks_logged_out(self, text, url=""):
        ...
        return (urlsplit(url).path.endswith(("verTelaLogin.do", "logar.do"))
                or BeautifulSoup(text, "lxml").select_one('input[type="password"], form[name="loginForm"]') is not None)
