...
def register(sub):
    ...
    login = commands.add_parser("login-probe", help="fingerprint a public login form; does not log in")
    ...
    cap = commands.add_parser("capture", help="capture private raw pages from the selected account")
...
def login_probe(args, settings=None):
    ...
    with httpx.Client(follow_redirects=True, timeout=30,
                      event_hooks={"request": [validate]}) as client:
...
def run_capture(args, settings):
    directory, manifest = capture(settings, output=args.out, include_matricula=args.include_matricula)
    result = {"directory": str(directory), "captured": sum(e["status"] == "captured" for e in manifest["entries"]),
              "nav_failed": sum(e["status"] == "nav_failed" for e in manifest["entries"])}
