...
from sigaa.config import (
    KEYRING_ACTIVE_INSTITUTION,
    KEYRING_SETTINGS_SERVICE,
    Settings,
    default_db_path,
)
from sigaa.errors import UnsafeUrlError, UnsupportedFeatureError, error_stage
...
def _session(handler, navigator=None):
    provider = get("ufpb")
    ...
    session = Session(
        ...
        navigator=navigator or provider.navigator,
    )
...
@pytest.mark.parametrize("url", [
    ...
    "https://user@sigaa.ufpb.br/",
    "https://sigaa.ufpb.br:8443/",
])
...
def test_each_gated_command_declares_its_capability(argv, capability):
    assert cli._build_parser().parse_args(argv).capability is capability
...
def test_unsupported_store_command_does_not_open_the_store(
    other, command, monkeypatch, tmp_path, capsys
):
    monkeypatch.setenv("SIGAA_INSTITUTION", other.key)
...
def test_unsupported_mcp_tool_does_not_open_the_store(other, monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("SIGAA_INSTITUTION", other.key)
    monkeypatch.setenv("SIGAA_DB", str(tmp_path / "never-created.db"))
    tools = (mcp_server.sigaa_list_grades, mcp_server.sigaa_list_news, mcp_server.sigaa_export_ics)
    for tool in tools:
...
def test_blocked_http_hop_names_scheme_and_host_and_stages_as_network():
    ...
    with _session(handler) as session, pytest.raises(UnsafeUrlError) as caught:
        session.get(get("ufpb").profile.portal_entry_url)
